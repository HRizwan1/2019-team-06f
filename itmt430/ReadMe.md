...
1) Issue the command ```packer build --var-file=./variables.json ubuntu1810-itmt430-database.json``` and ```packer build --var-file=./variables.json ubuntu1810-itmt430-webserver.json``` to begin the install with password, usernames, and RSA private key properly seeded
